...
-- Source Table: sample_insurance_transaction_daily
-- =============================================================================
...
FROM
  sample_insurance_transaction_daily
...
-- Sources     : sample_insurance_whitelist_daily & sample_insurance_transaction_daily
...
WITH whitelist_recent AS (
  ...
  FROM sample_insurance_whitelist_daily
...
txn_recent AS (
  ...
  -- Calculates Recency (last transaction date) and Frequency (txn count) metrics.
  SELECT
    msisdn,
    COUNT(DISTINCT transaction_id) AS txn_count_90d,
    MAX(dt_id) AS last_txn_date
  FROM sample_insurance_transaction_daily
  WHERE dt_id >= CURRENT_DATE - INTERVAL '90 days'
  GROUP BY msisdn
),
